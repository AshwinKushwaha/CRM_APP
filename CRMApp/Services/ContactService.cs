...
namespace CRMApp.Services
{
    public interface IContactService
    {
		...
		(List<CustomerContact> contacts, int contactCount) GetAllContacts(int pageIndex);
        CustomerContact GetContact(int id);
        bool CreateContact(CustomerContact contact);
        bool DeleteContact(int id);
        int GetContactCount();
        Task<ApplicationUser> GetCurrentUserAsync();
        List<CustomerContact> GetContacts(ContactFilter contactFilter, string input, int? customerId, int pageIndex);
    }
    public class ContactService : IContactService
    {
        ...
        public (List<CustomerContact> contacts, int contactCount) GetAllContacts(int pageIndex)
        {
            var contacts = context.CustomerContacts
            .Skip((pageIndex - 1) * PageSize).Take(PageSize).ToList();

            var count = context.CustomerContacts.Count();
        ...
        public List<CustomerContact> GetContacts(ContactFilter contactFilter, string input, int? customerId, int pageIndex)
        {
            if (customerId != null)
            {
                switch (contactFilter)
                {
                    case ContactFilter.Contact:
                        return context.CustomerContacts.Where(c => (!string.IsNullOrEmpty(input)) && (c.CustomerId == customerId) && (c.Contact.Contains(input)))
                                .Skip((pageIndex - 1) * PageSize).Take(PageSize).ToList();
                    case ContactFilter.ContactName:
                        return context.CustomerContacts.Where(c => (!string.IsNullOrEmpty(input)) && (c.CustomerId == customerId) && (c.ContactName.Contains(input)))
                                .Skip((pageIndex - 1) * PageSize).Take(PageSize).ToList();
                    case ContactFilter.Relation:
                        _ = Enum.TryParse(input, true, out Relation parsedInput);
                        return context.CustomerContacts.Where(c => (!string.IsNullOrEmpty(input)) && (c.CustomerId == customerId) && (c.Relation == parsedInput))
                            .Skip((pageIndex - 1) * PageSize).Take(PageSize).ToList();
                    case ContactFilter.ContactType:
                        _ = Enum.TryParse(input, true, out Models.Type parsedContactInput);
                        return context.CustomerContacts.Where(c => (!string.IsNullOrEmpty(input)) && (c.CustomerId == customerId) && (c.ContactType == parsedContactInput))
                            .Skip((pageIndex - 1) * PageSize).Take(PageSize).ToList();
                    case ContactFilter.All:
                    default:
                        return context.CustomerContacts.Where(c => (!string.IsNullOrEmpty(input)) &&
            ...
            else
            {
                ...
                {
                    case ContactFilter.Contact:
                        return context.CustomerContacts.Where(c => (!string.IsNullOrEmpty(input)) && (c.Contact.Contains(input)))
                            .Skip((pageIndex - 1) * PageSize).Take(PageSize).ToList();
                    case ContactFilter.ContactName:
                        return context.CustomerContacts.Where(c => (!string.IsNullOrEmpty(input)) && (c.ContactName.Contains(input)))
                            .Skip((pageIndex - 1) * PageSize).Take(PageSize).ToList();
                    case ContactFilter.Relation:
                        _ = Enum.TryParse(input, true, out Relation parsedInput);
                        return context.CustomerContacts.Where(c => (!string.IsNullOrEmpty(input)) && (c.Relation == parsedInput))
                            .Skip((pageIndex - 1) * PageSize).Take(PageSize).ToList();
                    case ContactFilter.ContactType:
                        _ = Enum.TryParse(input, true, out Models.Type parsedContactInput);
                        return context.CustomerContacts.Where(c => (!string.IsNullOrEmpty(input)) && (c.ContactType == parsedContactInput))
                            .Skip((pageIndex - 1) * PageSize).Take(PageSize).ToList();
                    case ContactFilter.All:
                    default:
                        return context.CustomerContacts.Where(c => (!string.IsNullOrEmpty(input)) &&
